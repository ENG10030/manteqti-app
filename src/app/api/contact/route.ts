...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!allowed) {
      return NextResponse.json({ error: 'طلبات كثيرة جداً، حاول بعد 15 دقيقة' }, { status: 429 });
    }
    ...
    if (!name || typeof name !== 'string' || name.trim().length < 2) {
      return NextResponse.json({ error: 'يجب أن يكون الاسم حرفين على الأقل' }, { status: 400 });
    }
    ...
    if (!message || typeof message !== 'string' || message.trim().length < 10) {
      return NextResponse.json({ error: 'يجب أن تكون الرسالة 10 أحرف على الأقل' }, { status: 400 });
    }

    // MEDIUM FIX: Sanitize ALL fields including message (was only stripping name before)
    const sanitized = {
